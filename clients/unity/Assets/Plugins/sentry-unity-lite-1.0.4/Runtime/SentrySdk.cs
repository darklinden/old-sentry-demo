...
public class SentrySdk : MonoBehaviour
{
    ...
    private Coroutine DoCaptureMessage(string message)
    {
        if (Debug)
        {
            UnityDebug.Log("sending message to sentry: " + message);
        }
    ...
    private IEnumerator<UnityWebRequestAsyncOperation> ContinueSendingEvent<T>(T @event) where T : SentryEvent
    {
        ...
        UnityEngine.Debug.Log("URL:" + _dsn.callUri.ToString());
        UnityEngine.Debug.Log("Header: X-Sentry-Auth:" + _stringBuilder.ToString());
        UnityEngine.Debug.Log("Body:" + s);
